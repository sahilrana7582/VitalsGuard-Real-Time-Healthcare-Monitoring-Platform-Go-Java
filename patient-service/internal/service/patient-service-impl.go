...
func (s *patientService) CreatePatient(ctx context.Context, p dto.NewPatientDTO) (*dto.PatientDTOResponse, error) {
...
func (s *patientService) CreatePatientProfile(ctx context.Context, p dto.NewPatientProfileDTO) (*dto.PatientProfileDTOResponse, error) {

	if _, err := time.Parse("2006-01-02", p.DOB); err != nil {
		return nil, errors.New("dob must be in YYYY-MM-DD format")
	}
	if err := s.repo.CreatePatientProfile(ctx, p); err != nil {
...
func (s *patientService) CreateVitalsInfo(ctx context.Context, v dto.NewVitalsInfoDTO) (*dto.VitalsInfoDTOResponse, error) {

	if _, err := time.Parse(time.RFC3339, v.RecordedAt); err != nil {
		return nil, errors.New("recorded_at must be in RFC3339 format")
	}
	if err := s.repo.CreateVitalsInfo(ctx, v); err != nil {
